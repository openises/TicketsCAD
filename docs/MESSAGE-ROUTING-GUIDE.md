...
> **Where did this rule come from?** If you see a rule here named "Chat
> Bridge → Slack" (or Telegram/Email/Mesh Radio) that you don't remember
> creating, it was generated by the corresponding checkbox under
> **Settings → Chat Settings → Cross-platform Bridges**. You can edit its
> filters/transform here like any other rule — see
> [Chat Bridge](CHAT-BRIDGE.md) for what that toggle does and the
> direct-message exclusion that applies to it (and to any local-chat rule
> you build yourself).
