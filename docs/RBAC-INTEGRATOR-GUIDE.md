...
- **(2026-08-22) Set `permissions.admin_only` on every new admin-only code — this is now the REQUIRED step, the exclusion-list bullet above is secondary.** After a fifth incident of this exact bug class (`action.manage_calls`'s canonical alias leaking onto Dispatcher via the A8 mirror step, in the SAME COMMIT that created the permission and its own exclusion-list entry — proof that remembering the exclusion-list convention above isn't enough), "admin-only" became a real schema column instead of only a hand-maintained string list. See `inc/rbac_admin_only.php`'s docblock for the two-tier model (1 = Org Admin or above, 2 = Super Admin only) and why a plain boolean can't represent both. When adding a new admin-only permission: (1) add it to the exclusion list(s) as before — this is still what determines the DEFAULT seed, (2) set `admin_only` to the correct tier in the classification block near the bottom of `sql/rbac.sql` and the matching block in `sql/run_00_rbac.php` (both — they intentionally duplicate each other, same convention as the exclusion lists themselves). Once `admin_only` is set, `api/rbac.php`'s live grant endpoints and `sql/run_rbac_v2.php`'s A8 mirror step will REFUSE to grant that code to an under-tier role even if a future edit forgets the exclusion-list text — the column is the backstop, not a replacement for the list. `tools/rbac_exclusion_leak_audit.php`'s Part 0 checks the live database against `admin_only` directly (its most authoritative check now) and cross-references it against the exclusion-list text for drift.
