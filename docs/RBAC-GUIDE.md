...
- **An Org Admin or Dispatcher can do something that table says they shouldn't be able to** (an admin-only action from the tables above succeeds for them) — this is a real class of bug this project fixed 2026-08-16 (and hardened further 2026-08-22, see below): the default Org Admin/Dispatcher grant is seeded as "everything except a literal list of excluded codes," and that list can leak several ways — a role holding an excluded code's canonical alias (created later by the RBAC v2 migration, which a list written earlier can't have named), a role that was granted the code directly before it was ever added to the exclusion list, or (2026-08-22) the RBAC v2 canonical-alias mirror step handing a role a grant that no exclusion list ever authorized in the first place. All are now self-healing on every re-seed (`php sql/run_00_rbac.php` or a fresh `sql/rbac.sql` import repairs the first two; the third can no longer happen at all, structurally, once `permissions.admin_only` is set — see `inc/rbac_admin_only.php`), but if you're on an install that hasn't re-run the seed since 2026-08-22, re-run it. Run `php tools/rbac_exclusion_leak_audit.php` to check the live database directly — its "admin_only column check" is the most authoritative signal. `tests/test_rbac_canonical_alias_leak.php` and `tests/test_rbac_admin_only.php` are the regression coverage.
